...
The NimbleBrain API provides programmatic access to all Studio features. Use it to chat with Nira, execute playbooks, and build custom integrations.
...
curl -H "Authorization: Bearer nb_live_xxxxx" \
  https://studio-api.nimblebrain.ai/v1/nira/conversations
```
...
|--------|----------|-------------|
| `POST` | `/v1/nira/conversations` | Create conversation with Nira |
| `GET` | `/v1/nira/conversations/:id` | Get conversation details |

### Nira Messages

| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/v1/nira/conversations/:convId/messages` | List messages |
| `POST` | `/v1/nira/conversations/:convId/messages` | Send message to Nira |
