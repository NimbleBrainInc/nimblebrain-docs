...
<Aside type="note">
  The `PORT` environment variable takes precedence over `--port`. If both are set, `PORT` wins.
</Aside>
...
Authentication is configured through `instance.json` (the `oidc` or `workos` adapter), not an environment variable — there is no `NB_API_KEY`. `ALLOWED_ORIGINS` controls CORS. See [Connect via MCP](/connect/mcp-endpoint/) and [Security](/deploy/security/) for details.
