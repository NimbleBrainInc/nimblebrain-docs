...
When a NimbleBrain server is running on the default port (3000), `nb status` also queries the `/v1/bundles/health` endpoint and shows live bundle health:
...
<Aside type="note">
  If no server is running, the health section is omitted. The command checks `http://localhost:3000/v1/bundles/health` with a 2-second timeout.
</Aside>
