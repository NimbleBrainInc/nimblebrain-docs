...
| `credential` (alias `creds`) | Manage workspace-scoped secrets (OAuth client secrets, API keys) |
| `user` | User account management (no subcommands yet) |
...
Commands:
  ...
  telemetry    Manage anonymous usage telemetry
  automation   Manage scheduled automations
  credential   Manage workspace-scoped secrets
  user         User account management (no subcommands yet)
...
All modes (interactive, `nb serve`, and `nb dev`) default to the same global work directory at `~/.nimblebrain`. `nb dev` spawns `nb serve` under the hood, so it inherits that default unless you override it with `--config`, `--workdir`, or `NB_WORK_DIR`.
...
- [nb credential](/cli/credential/) -- manage workspace-scoped secrets
