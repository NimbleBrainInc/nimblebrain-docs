...
The `/mcp` endpoint turns NimbleBrain into a **Streamable HTTP MCP server**. It is NimbleBrain's only public programmatic surface: external MCP clients (Claude Desktop, Claude Code, Cursor, another NimbleBrain instance, or any MCP-compatible host) connect here and access the tools installed across your workspaces.
...
- [Auth adapters](/config/instance-json) — `dev` / `local` / `oidc` / `workos` configuration in `instance.json`.
