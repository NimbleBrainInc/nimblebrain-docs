...
| Tool | Description |
|------|-------------|
| `nb__search_tools` | Search available tools by keyword across all installed bundles |
| `nb__search_registry` | Search the mpak registry for bundles to install |
...
| Total tools > 30, no skill matched | Only `nb__*` system tools. Others available via `nb__search_tools`. |
| Skill matched with `allowed-tools` | Tools matching the globs + system tools. Others via `nb__search_tools`. |

The threshold is configurable via `maxDirectTools` (default: 30).

<Aside type="tip">
When you have many bundles installed, the agent uses `nb__search_tools` to find the right tool before calling it. You don't need to manage this — the bootstrap skill teaches the agent how to use meta-tools automatically.
</Aside>
...
This means NimbleBrain is both an MCP **client** (connecting to installed bundles) and an MCP **server** (exposing composed tools to external hosts). See [MCP Endpoint](/api/mcp-endpoint) for configuration and usage.
