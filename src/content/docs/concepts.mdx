...
| Source | How it works |
|--------|-------------|
| `name` | Downloaded from the [mpak](https://mpak.dev) registry. The agent can install these at runtime via `nb__manage_app`. |
...
| `protected` | Prevents uninstall via the `nb__manage_app` system tool |
