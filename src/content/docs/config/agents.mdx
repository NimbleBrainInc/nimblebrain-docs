...
| `maxIterations` | `integer` | No | — | Maximum iterations for this agent. Falls through to the `nb__delegate` call's default (5) when omitted. The effective value is capped at the **child ceiling of 10** and at the parent's remaining budget minus 1, so a profile value above 10 is clamped to 10. |
