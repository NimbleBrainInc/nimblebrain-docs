...
| `sessionStore` | `object` | `{ "type": "memory", "ttlMs": 28800000 }` | MCP session metadata store for `/mcp`. See [sessionStore](#sessionstore). |
...
| `ttlMs` | `integer` (ms) | `28800000` (8 h) | Idle TTL. Each request resets the clock — actively-used sessions never expire. |
...
The 8 h default is tuned for a connector left open during a working day. Tighten it (e.g. `1800000` for 30 minutes) if you want sessions to evict faster on idle; loosen it if your typical usage pattern leaves connectors open across days.

`MCP_SESSION_TTL_MS` env var overrides this when set.
