...
| `NB_CATALOG_PATH` | No | — | Override path for the OAuth connectors catalog. YAML or JSON. Replaces the bundled catalog. See [Connectors Catalog](/config/connectors-catalog/#operator-override). |
| `NB_STDIO_CATALOG_PATH` | No | — | Override path for the curated stdio bundle list. YAML or JSON. Same Replace-not-Merge semantics. |
