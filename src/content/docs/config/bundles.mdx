...
<Aside type="tip">
  For sensitive values, consider using the `nb config set` command instead of putting secrets directly in the config file. Values set via `nb config set` are mapped to environment variables through the bundle manifest's `user_config` declarations.
</Aside>
