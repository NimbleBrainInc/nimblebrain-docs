...
description: Configure named, local, and remote MCP bundles in workspace.json.
---

import { Tabs, TabItem, Aside } from '@astrojs/starlight/components';

The `bundles` array lives in [`workspace.json`](/config/workspace-json/), **not** `nimblebrain.json`. Each workspace declares its own bundles; a `bundles` array placed at the top level of `nimblebrain.json` is silently stripped on load. This page documents the shape of a single bundle entry — the same shape whether the entry was written by hand, by the connectors install flow, or by the CLI.
...
<Aside type="note">
  Bundles are workspace-scoped. Two workspaces that install the same bundle spawn independent subprocesses with separate data directories. See [`workspace.json`](/config/workspace-json/) for where the array lives and how workspaces are managed.
</Aside>
