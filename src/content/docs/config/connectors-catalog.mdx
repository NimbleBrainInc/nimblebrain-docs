...
- **`bundled-static`** (locked, type `static`) — the curated list shipped at `src/connectors/curated/example.yaml`. Cannot be disabled or removed; operators may rename it.
...
Append an entry to the bundled catalog at `src/connectors/curated/example.yaml`. The file is a list of upstream `ServerDetail` records; platform-specific fields live under the reverse-DNS extension key `_meta["ai.nimblebrain/connector"]`.
...
CI runs this probe automatically on PRs that touch the bundled catalog (`.github/workflows/catalog-check.yml`), so a vendor going down or dropping DCR support breaks the PR before the catalog ships. It's not part of `bun run verify` (which stays offline).
