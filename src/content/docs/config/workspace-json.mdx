...
<Aside type="tip">
  You rarely edit `workspace.json` by hand under auth. Use `nb__manage_workspaces` (admin-only) to create workspaces and update membership; install bundles through the connectors catalog (`manage_connectors` / `manage_apps`) or the CLI; use the workspace-scoped skill tools to manage `skillDirs`.
</Aside>
