...
<Aside type="note">
  `bundleManagement` is **reserved**. Its `nb__manage_app` tool was removed — bundle install and configuration now happen through the connectors catalog (`manage_connectors`, `manage_apps`) and the CLI. The flag is kept as a stable config knob for a future bundle-management tool, but toggling it has no effect on the current tool set.
</Aside>
