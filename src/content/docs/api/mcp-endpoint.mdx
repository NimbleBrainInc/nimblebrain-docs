...
|---------|---------|---------|-------------|
| Max concurrent sessions | `MCP_MAX_SESSIONS` | `100` | Soft cap. A new initialize at capacity evicts the least-recently-used session and is admitted; it never fails with `429`. |
| Session TTL | `MCP_SESSION_TTL_SECONDS` | `28800` (8 h) | Idle timeout. Each request resets the clock — an actively-used session never expires. Idle sessions are closed by a periodic sweep on the serving pod. |
...
See [`sessionStore` config](/config/nimblebrain-json#sessionstore) for shape, providers, and operator guidance.
...
| Reason | Meaning |
|--------|---------|
| `not_found` | Session store has no record. Most often: idle-TTL eviction, or the session never existed. |
| `unavailable` | Session exists in the store, but the live transport isn't on this process. Possible causes: process restart since `initialize`, sticky-routing miss, or local transport closure. |
...
- **Session limits** — `MCP_MAX_SESSIONS` bounds memory; the LRU eviction policy means a runaway client cannot deny service to others.
