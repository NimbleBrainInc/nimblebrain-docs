...
description: NimbleBrain is a self-hosted platform for MCP Apps and agent automations — install MCP servers as apps, compose tools across them, render MCP Apps with full ext-apps support, and expose the whole platform as an MCP server.
---

import { Aside } from '@astrojs/starlight/components';

NimbleBrain is a **self-hosted platform for MCP Apps and agent automations**. It aggregates multiple MCP servers into a unified tool namespace, renders interactive [MCP Apps](https://modelcontextprotocol.io/extensions/apps/overview) inside the web client, and exposes the entire platform as an MCP server that other hosts can connect to.
...
| **System Tools** | Built-in tools for discovery (`nb__search`/`nb__status`), multi-agent delegation (`nb__delegate`), and platform management (`manage_connectors`, `manage_apps`, `manage_workspaces`, `manage_users`) |
...
<CardGrid>
  <LinkCard title="New user?" description="Learn how to chat, use apps, and navigate the web UI" href="/guide/welcome" />
  <LinkCard title="Setting up NimbleBrain?" description="Install, configure, and deploy the platform" href="/quickstart" />
  <LinkCard title="Building an app?" description="Create MCP bundles that run inside NimbleBrain" href="/apps/overview" />
