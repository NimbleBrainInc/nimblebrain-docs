...
| Tool | Purpose |
|------|---------|
| `nb__search_tools` | Search available tools by keyword across all installed apps |
| `nb__search_registry` | Search the mpak registry for new bundles to install |
...
<Aside type="tip">
  When more than 30 tools are installed, NimbleBrain uses tiered tool surfacing. Only system tools and matched skill tools are surfaced directly. The agent uses `nb__search_tools` to find and call other tools as needed.
</Aside>
