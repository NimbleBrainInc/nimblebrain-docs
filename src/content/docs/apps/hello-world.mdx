...
function HelloApp() {
  ...
  // The latest tool result delivered by the host (agent-initiated calls).
  // `useToolResult()` takes no arguments and returns the most recent result.
  const lastResult = useToolResult();
  useEffect(() => {
    if (lastResult) setGreeting(String(lastResult.content));
  }, [lastResult]);
  ...
  async function greet() {
    ...
    setCount(Number(countResult.data));
  }
