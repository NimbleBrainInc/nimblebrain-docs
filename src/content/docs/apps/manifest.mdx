...
description: Complete reference for the _meta["ai.nimblebrain/platform"] manifest extension.
---

import { Aside } from '@astrojs/starlight/components';

NimbleBrain reads platform-specific metadata from the `_meta["ai.nimblebrain/platform"]` key in your MCPB `manifest.json`. This is where you declare your app's name, icon, primary view, sidebar placements, and settings section.
...
| `_meta` | `object` | No | Extension metadata. NimbleBrain reads `_meta["ai.nimblebrain/platform"]`. |
...
## Platform metadata: `_meta["ai.nimblebrain/platform"]`

### `ui` object

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `ui.name` | `string` | Yes | Display name shown in the sidebar and app list. |
| `ui.icon` | `string` | Yes | [Lucide icon](https://lucide.dev/icons) name in kebab-case (e.g., `"database"`, `"message-square"`, `"hand"`). |
| `ui.primaryView` | `object` | No | The main view loaded when a user navigates to your app. |
| `ui.primaryView.resourceUri` | `string` | Yes (if `primaryView` set) | `ui://` URI for the main view (e.g., `"ui://dashboard"`). |
| `ui.placements` | `PlacementDeclaration[]` | No | Explicit shell layout placement declarations. See [Placements](/apps/placements). |
...
Each entry in the `ui.placements` array:
...
If your manifest has `ui.placements`, those are used directly. If it only has `ui.primaryView` (no `placements` array), NimbleBrain converts it to a single `"main"` slot placement automatically:
