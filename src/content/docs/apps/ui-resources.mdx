...
NimbleBrain's own surfaces — the home dashboard, conversation list, usage, files, and automations — use the exact same `ui://` pattern as third-party apps. There is no separate "core" code path. Each built-in is an **in-process MCP bundle**: it runs inside the platform process (over an in-memory transport) and registers `ui://` resources just like a subprocess bundle would.

When the resource proxy receives a request for a built-in's view, it calls `resources/read` on that in-process bundle through the same dispatch path it uses for any other server:

```
GET /v1/apps/home/resources/primary   → home bundle's ui:// resource
