...
<Aside type="caution">
**Live-update caveat: only ext-apps spec tokens update on toggle.** The initial `<style>` block injected at load carries the full token set, including the `--nb-*` extensions. But the `ui/notifications/host-context-changed` payload sent on a dark-mode toggle carries **only the spec-standard tokens** (the `--color-*`, `--font-*`, `--border-radius-*`, `--shadow-*` keys in the table below). The `--nb-*` extension tokens are filtered out of that payload because the ext-apps `styles.variables` field is a typed enum and strict SDK clients reject unknown keys.

In practice: drive any color that must flip between light and dark from the spec `--color-*` tokens. The `--nb-*` tokens are safe for one-time styling at load, but they will not refresh live on a theme toggle, so don't depend on them for light/dark-sensitive surfaces.
</Aside>

## Token reference

The platform injects the following tokens into every iframe. Spec tokens (everything except the `--nb-*` block) also cross the ext-apps protocol boundary and refresh live on theme changes; `--nb-*` extensions are injected at load only.
...
| `--color-text-accent` | `#0055FF` | `#3b8eff` | Links, primary buttons, AI accents |
...
| `--font-mono` | `'JetBrains Mono Variable', ui-monospace, monospace` | Code / monospace font |
