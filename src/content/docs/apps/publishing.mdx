...
Add NimbleBrain host metadata in `_meta["ai.nimblebrain/host"]` if your app has a UI. See [Manifest Reference](/apps/manifest) for the full schema.
...
After publishing, anyone can install your app from the Apps catalog in their workspace settings, or by adding it to their `workspace.json` bundles and running `nb reload`.
...
2. **Add your bundle** by path in `nimblebrain.json`:
   ...
   Then run `nb reload` (or restart the dev server) to load it.
