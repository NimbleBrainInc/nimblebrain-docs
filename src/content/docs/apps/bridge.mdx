...
| `synapse/request-file` | App → Host | Open native file picker; the host persists each chosen file to the workspace store and returns `{ id, filename, mimeType, size }`. |
...
Read a resource from your app's MCP server. This is a JSON-RPC **request** — the response is a standard MCP `ReadResourceResult` (`{ contents: [...] }`). The URI passes through verbatim to your server; resources are namespaced by the bundle that authored them.
...
The platform's built-in apps ship a small bridge runtime script that handles theme synchronization automatically. It listens for `ui/initialize` (legacy notification path) and `ui/notifications/host-context-changed` (spec) and applies tokens as CSS custom properties.
