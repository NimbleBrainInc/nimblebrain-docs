...
A NimbleBrain app is an [MCPB bundle](https://github.com/modelcontextprotocol/mcpb) — a packaged MCP server with a `manifest.json`. Every app exposes **tools** that the agent can call. Apps can optionally provide **UI** rendered inside the NimbleBrain web client as sandboxed iframes.
