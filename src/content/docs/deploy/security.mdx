...
<Aside type="danger">
Running without `NB_API_KEY` disables all authentication. The CORS policy falls back to `Access-Control-Allow-Origin: *`. Anyone who can reach the server has full access to the agent, including shell execution via the bash bundle.
</Aside>
...
2. Update the `NB_API_KEY` environment variable in your deployment (`.env` file, Helm values, or Kubernetes Secret).
...
The platform service (port 3000) should never be directly exposed to the internet. In both Docker Compose and Kubernetes deployments, the platform runs on an internal network:

- **Docker Compose**: The platform has no `ports` mapping. Only the `web` container (Caddy) is exposed on port 8080. Caddy proxies `/v1/*` to the platform.
- **Kubernetes**: Both services are `ClusterIP`. The Ingress routes `/v1/*` to the platform and `/` to the web pod.

```
Internet → Reverse Proxy (TLS) → Web (8080) → Platform (3000, internal)
                                       ↑
                                  Serves UI + proxies /v1/*
```

<Aside type="danger">
Do not add `ports: - "3000:3000"` to the platform service in `docker-compose.yml`. This would expose the API directly, bypassing the web container's reverse proxy.
</Aside>
...
ufw allow 22/tcp
ufw allow 443/tcp
ufw deny 8080/tcp   # Block direct access if behind a reverse proxy
ufw enable
```
...
<TabItem label="Kubernetes Ingress">
...
| Item | How to verify |
|---|---|
| `NB_API_KEY` set to 32+ characters | Check your `.env` or Helm values |
...
| Platform port (3000) not exposed | `docker compose ps` shows no host port mapping for platform |
