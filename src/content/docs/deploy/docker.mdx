...
NimbleBrain ships two container images: **platform** (agent engine + API on port 3000) and **web** (React UI served by Caddy on port 8080). Docker Compose runs both on a single machine with a shared internal network.
...
| `web` | `nimblebrain/web` | 8080 (exposed) | React SPA served by Caddy, proxies `/v1/*` to platform |
...
<Aside type="caution">
Running without `NB_API_KEY` disables authentication entirely. CORS falls back to `Access-Control-Allow-Origin: *`. This is acceptable for local development but never for production.
</Aside>
...
3. **Verify**
   ...
   ```
   NAME                   STATUS                 PORTS
   project-platform-1     running (healthy)      3000/tcp
   project-web-1          running                0.0.0.0:8080->8080/tcp
   ```
...
4. **Open the UI**
   ...
   Go to [http://localhost:8080](http://localhost:8080). Enter your `NB_API_KEY` value to log in.
...
The `nimblebrain/platform` image is built from `oven/bun:1.1` and includes:
...
The entrypoint is `bun run src/cli/index.ts serve`, which starts the HTTP API server on port 3000 with `NB_WORK_DIR=/data` and `NB_HOST=0.0.0.0`.

## Reverse proxy

The web container's Caddy server already handles reverse proxying for the default setup. If you want to place your own reverse proxy in front of the stack (for TLS termination, custom domain, etc.), point it at port 8080:
