...
description: 'Generate and manage authentication tokens for CLI and programmatic access'
icon: 'key'
---

Generate workspace tokens for CLI authentication, API access, and CI/CD pipelines.
...
<Tabs>
  <Tab title="Standard Token">
    **Expiration**: 30 days
    **Use Case**: Personal CLI usage, development
  ...
  <Tab title="Long-Lived Token">
    **Expiration**: 1 year
    **Use Case**: CI/CD pipelines, production services
    **Refresh**: Cannot be refreshed (regenerate when expired)

    Best for automation that shouldn't require frequent updates
  </Tab>
  <Tab title="Read-Only Token">
    **Expiration**: 30 or 365 days
    **Use Case**: Monitoring, metrics collection
    **Permissions**: View-only access (cannot deploy or modify)

    Best for dashboards and monitoring tools
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="Configure Token">
    ...
    - **Name/Description**: Identify the token's purpose
    - **Type**: Standard, Long-Lived, or Read-Only
    - **Expiration**: Auto-set based on type (or custom)
  </Step>
  <Step title="Generate & Copy">
    Click **Generate Token** and **immediately copy it**
  </Step>
</Steps>
...
<Steps>
  <Step title="Locate Token">
    Find the token in Settings → API Tokens
  </Step>
...
<Callout type="danger">
  Revoking a token immediately stops all services using it. Update integrations with a new token first!
</Callout>
